...
class SpeakerProtectionSwitch(context: Context) : OnPreferenceChangeListener {
    ...
    override fun onPreferenceChange(preference: Preference?, newValue: Any): Boolean {
        ...
        Utils.setSystemProp(SpeakerProtectionSwitch.SYSTEM_PROP, if (enabled) "true" else "false")
        return true
    }

    companion object {
        const val SETTINGS_KEY = DeviceSettings.KEY_SETTINGS_PREFIX + DeviceSettings.KEY_SPEAKER_PROTECTION
        const val SYSTEM_PROP = "persist.vendor.audio.speaker.prot.enable"
        fun isCurrentlyEnabled(context: Context?): Boolean {
            if(Utils.getSystemProp(SYSTEM_PROP).equals("true"))
            {
                return true
            }else {
                return false
        ...
        fun restore(storedValue: String) {
            Utils.setSystemProp(SYSTEM_PROP,storedValue);
        }
